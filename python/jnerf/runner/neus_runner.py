...
class NeuSRunner:
    ...
    def validate_mesh(self, world_space=False, resolution=64, threshold=0.0, name=None, auto_load=False):
        if auto_load:
            model_list_raw = os.listdir(os.path.join(self.base_exp_dir, 'checkpoints'))
            model_list = []
            for model_name in model_list_raw:
        ...
        if not name:
            ...
            mesh.export(os.path.join(self.base_exp_dir, '{}.ply'.format(name)))
        logging.info('End')
