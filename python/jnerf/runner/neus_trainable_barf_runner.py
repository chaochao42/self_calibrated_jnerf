...
class NeuS_Trainable_Barf_Runner:
    def __init__(self, mode='train', is_continue=False):
        ...
        print(f"=============dataset pose type {type(self.dataset.pose_all)}")
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        print("=====================Previous")
        print(self.dataset.pose_all[0])
        self.dataset.pose_all = jt.Var(checkpoint['trained_pose'])
        print("=====================Loaded")
        print(self.dataset.pose_all[0])
        print(f"=========type neus {type(checkpoint['neus'])}")
        print(f"=========type iter {type(checkpoint['iter_step'])}")
        print(f"=========type ckpt {type(checkpoint['trained_pose'])}")
        logging.info('End')
    ...
    def validate_debug_image(self, idx=-1, auto_load=False, resolution_level=-1):
        if auto_load:
            model_list_raw = os.listdir(os.path.join(self.base_exp_dir, 'checkpoints'))
            model_list = []
            for model_name in model_list_raw:
        ...
        print('Generation: iter: {}, camera: {}'.format(self.iter_step, idx))
        ...
        for i in range(img_fine.shape[-1]):
            if len(out_rgb_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                                        'rgb_{}_{}.png'.format(i, idx)),
                           np.concatenate([img_fine[..., i],
                                           self.dataset.image_at(idx, resolution_level=resolution_level)]))
            if len(out_normal_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                                        'normal_{}_{}.png'.format(i, idx)),
                           normal_img[..., i])

            if len(out_depth_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                                        'depth_{}_{}.png'.format(i, idx)),
                           depth_fine[..., i])
